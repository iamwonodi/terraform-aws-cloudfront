...
* Automatic S3 Origin Access Control creation
...
  source = "git::https://github.com/iamwonodi/terraform-aws-cloudfront.git?ref=v1.2.0"
...
For S3 origins, the module can create and attach the Origin Access Control
automatically:
...
When enabled, the module creates an S3 OAC using SigV4 signing with signing
behavior set to `always` and attaches it to all S3 origins.

The generated OAC is exposed through:

```hcl
module.cloudfront.s3_origin_access_control_id
module.cloudfront.s3_origin_access_control_arn
```

The caller remains responsible for the S3 bucket policy. The recommended
policy grants `s3:GetObject` to the `cloudfront.amazonaws.com` service
principal and restricts access to the CloudFront distribution using
`AWS:SourceArn`.

Keep the S3 bucket private and use its regional REST endpoint rather than the
S3 website endpoint.
